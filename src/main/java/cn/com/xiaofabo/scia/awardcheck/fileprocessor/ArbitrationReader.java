...
import cn.com.xiaofabo.scia.awardcheck.entity.Pair;
import cn.com.xiaofabo.scia.awardcheck.entity.Proposer;
import cn.com.xiaofabo.scia.awardcheck.entity.Respondent;
import cn.com.xiaofabo.scia.awardcheck.entity.Routine;

public class ArbitrationReader extends DocReader {
	private final List<Proposer> proposerList;
	private final List<Respondent> respondentList;

	public ArbitrationReader() {
		super();
		proposerList = new LinkedList();
		respondentList = new LinkedList();
	}

	public Routine process(String inputPath) throws IOException {
		...
		for (int i = 0; i < lines.length; ++i) {
			...
			if (removeAllSpaces(line).equals("一、案情")) {
				fileNumText = line;
			...
			if (removeAllSpaces(line).equals("二、仲裁庭意见")) {
				caseTextLineEnd = i - 1;
			...
			if (removeAllSpaces(line).equals("三、裁决")) {
		...
		for (int i = 0; i < routineText.size(); ++i) {
			System.out.println(routineText.get(i));
		}
		System.out.println("---------------------------------------------------");
		for (int i = 0; i < caseText.size(); ++i) {
			System.out.println(caseText.get(i));
		}
		System.out.println("---------------------------------------------------");
		for (int i = 0; i < arbiOpinionText.size(); ++i) {
			System.out.println(arbiOpinionText.get(i));
		}
		System.out.println("---------------------------------------------------");
		for (int i = 0; i < arbitramentText.size(); ++i) {
			System.out.println(arbitramentText.get(i));
		}
